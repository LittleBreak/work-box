...
> Vue 2.7 → Vue 3 迁移技术文档（含全链路方案一 & 新旧共存方案二）
>
> 编写日期：2026-03-03
...
1. [项目现状](#1-项目现状)
2. [迁移策略与选型依据](#2-迁移策略与选型依据)
3. [迁移执行步骤](#3-迁移执行步骤)
   - [Phase 1: Webpack 3 → Webpack 5 + Babel 7](#phase-1-webpack-3--webpack-5--babel-7)
   - [Phase 2: Vue Router 3 → 4，Vuex 3 → 4](#phase-2-vue-router-3--4vuex-3--4)
   - [Phase 3: Vue 2.7 → @vue/compat（兼容模式）](#phase-3-vue-27--vuecompat兼容模式)
   - [Phase 4: @vue/compat → Vue 3（正式切换）](#phase-4-vuecompat--vue-3正式切换)
   - [Phase 5: Element UI → 新 UI 组件库](#phase-5-element-ui--新-ui-组件库)
4. [其他必须处理的迁移事项](#4-其他必须处理的迁移事项)
5. [工作量预估](#5-工作量预估)
6. [风险与应对策略](#6-风险与应对策略)
7. [方案二：@vue/compat 长期兼容（新旧共存）执行步骤](#7-方案二vuecompat-长期兼容新旧共存执行步骤)
   - [Phase A: Webpack 3 → Webpack 5 + Babel 7](#phase-a-webpack-3--webpack-5--babel-7)
   - [Phase B: Vue Router 3 → 4，Vuex 3 → 4](#phase-b-vue-router-3--4vuex-3--4)
   - [Phase C: 切换 @vue/compat + 双 UI 库共存](#phase-c-切换-vuecompat--双-ui-库共存)
...
| 技术项                | 当前版本            | 目标版本                             |
| --------------------- | ------------------- | ------------------------------------ |
| Node.js               | 16.20               | 18+ (LTS)                            |
| 包管理器              | pnpm 7.33.7         | pnpm 8+                              |
| Vue                   | 2.7.12              | 3.4+                                 |
| Vue Router            | 3.0.1               | 4.x                                  |
| Vuex                  | 3.1.2               | 4.x（或迁移至 Pinia）                |
| Element UI            | 2.13.0              | 替换为新 Vue 3 UI 库                 |
| Webpack               | 3.12.0              | 5.x                                  |
| Babel                 | 6.22.1 (babel-core) | 7.x (@babel/core)                    |
| vue-loader            | 13.3.0              | 17.x                                 |
| vue-template-compiler | 2.7.12              | 移除（Vue 3 内置 @vue/compiler-sfc） |
| Axios                 | 0.18.0              | 1.x                                  |
| Less                  | 3.9.0               | 4.x                                  |
| vue-i18n              | 8.14.1              | 9.x                                  |
...
| 包                                 | 当前版本 | 说明                                             |
| ---------------------------------- | -------- | ------------------------------------------------ |
| webpack-dev-server                 | 2.11.5   | 需升级到 4.x+                                    |
| webpack-merge                      | 4.2.2    | 需升级到 5.x+                                    |
| extract-text-webpack-plugin        | 3.0.0    | Webpack 5 中替换为 mini-css-extract-plugin       |
| uglifyjs-webpack-plugin            | 1.1.1    | Webpack 5 中替换为 terser-webpack-plugin（内置） |
| html-webpack-plugin                | 2.30.1   | 需升级到 5.x                                     |
| copy-webpack-plugin                | 4.0.1    | 需升级到 11.x+                                   |
| optimize-css-assets-webpack-plugin | 3.2.0    | 替换为 css-minimizer-webpack-plugin              |
| css-loader                         | 0.28.0   | 需升级到 6.x+                                    |
| less-loader                        | 5.0.0    | 需升级到 11.x+                                   |
| babel-loader                       | 7.1.1    | 需升级到 9.x+                                    |
...
**路径**：Webpack 升级 → 路由/状态管理升级 → @vue/compat 兼容过渡 → Vue 3 正式切换 → UI 库替换

**原因**：

1. **项目规模决定不可能一步到位**
   - 996 个 Vue 文件、124 个 Store 模块、77 个 API 模块，一次性重写意味着所有业务线全部停摆，在商业上不可接受。
   - 渐进式迁移允许业务开发与技术升级并行推进。

2. **@vue/compat 提供安全网**
   - `@vue/compat` 是 Vue 官方提供的 Vue 3 兼容构建版本，能在 Vue 3 运行时中模拟 Vue 2 行为。
   - 开启后项目可以在"半 Vue 2 半 Vue 3"状态下正常运行，每修完一处 Vue 2 用法就关闭一个兼容开关，风险可控。
   - 控制台会逐条输出废弃警告，提供明确的修改清单，不会遗漏。

3. **构建工具必须先行升级**
   - Vue 3 要求 `vue-loader` 16+，而 `vue-loader` 16+ 最低要求 Webpack 4，推荐 Webpack 5。
   - Vue 3 的模板编译器 `@vue/compiler-sfc` 依赖现代模块系统，Webpack 3 无法处理。
   - Babel 6 不支持 Vue 3 的 JSX 转换和可选链等语法，必须升级到 Babel 7。
   - 因此 Webpack + Babel 升级是一切后续步骤的前提。

4. **UI 库替换放在最后，避免二次迁移**
   - 如果在 Vue 2 阶段就换 UI 库，需要找一个同时兼容 Vue 2 和 Vue 3 的库，选择极少。
   - 在 @vue/compat 阶段，Element UI 2 仍可运行（兼容模式模拟 Vue 2 行为），无需提前替换。
   - 等 Vue 3 正式切换完毕后，直接从 Element UI 迁移到目标 Vue 3 UI 库，只做一次替换。

5. **每个阶段可独立验证和回滚**
   - 每个 Phase 完成后项目都应该能正常运行和通过测试。
   - 如果某个 Phase 出现不可控问题，可以回退到上一个稳定状态，不影响已完成的迁移工作。

### 2.2 方案二：@vue/compat 长期兼容（新旧共存）

**路径**：Webpack 升级 → 路由/状态管理升级 → @vue/compat 兼容 → **停留在 compat 模式，不全量迁移老页面**

**核心思路**：利用 @vue/compat 运行时就是 Vue 3 这一事实，新开发的页面直接使用 Vue 3 + 新 UI 库，存量老页面保持 Vue 2 写法 + Element UI 不动，两套代码在同一应用中共存。

**适用场景**：

1. **业务迭代压力大，无法投入大量人力做全量迁移**
   - 跳过方案一中 Phase 3 的 996 个文件逐一改造（20-30 天）和 Phase 5 的 UI 库全量替换（25-35 天）。
   - 总工作量从 60-89 天降低到约 15-20 天。

2. **只需新页面使用 Vue 3 特性**
   - 新页面可以使用 Composition API、`<script setup>`、Vue 3 专属 UI 库。
   - 老页面不触碰，不引入回归风险。

3. **渐进式按需迁移老页面**
   - 后续有时间时，可以逐个模块将老页面从 Vue 2 写法迁移到 Vue 3，最终达到方案一的终态。
   - 迁移进度完全自主可控，不存在"必须一次性全部完成"的压力。
...
| ---------- | ----------------------------- | ---------------------------- |
| 总工作量   | 60-89 人天                    | 15-20 人天                   |
| 老页面改动 | 全量改造 996 个文件           | 不动                         |
| 新页面能力 | Vue 3 + 新 UI 库              | Vue 3 + 新 UI 库             |
| Element UI | 最终移除                      | 老页面长期保留               |
...
工作量约 15-20 人天，仅为方案一的 1/4，聚焦构建工具链和核心框架升级，快速让团队用上 Vue 3 能力。

#### 2. 存量业务零改动，风险最低

老页面在兼容模式下原样运行，不触碰 996 个 Vue 文件和 124 个 Store 模块，从根本上避免大面积回归风险。

#### 3. 新页面立即使用 Vue 3

升级完成后，新页面可直接使用 Composition API、`<script setup>` 及 Vue 3 生态组件库，开发体验即刻提升。

#### 4. 业务开发与技术升级互不阻塞

老页面保持 Vue 2 + Element UI，新页面用 Vue 3 + 新 UI 库，两套代码共存，业务迭代无需为升级让路。

#### 5. 可平滑演进至方案一终态

方案二是方案一的子集，后续可结合业务迭代按模块逐步迁移老页面，节奏自主可控，无一次性完成的压力。

### 2.5 为什么不选择 Vite 替代 Webpack
...
### Phase 5: Element UI → 新 UI 组件库

**目标**：将 Element UI 2 替换为目标 Vue 3 UI 组件库。

> 以下以 Ant Design Vue / Arco Design Vue / Naive UI 等为假设目标，具体组件名以实际选型为准。
...
需要建立 Element UI → 新库的组件映射表，逐个替换。以下为通用映射示例：
...
## 4. 其他必须处理的迁移事项（方案一全量迁移需处理，方案二按需处理）
...
### 4.5 `this.$children` / `this.$listeners` 移除
...
## 7. 方案二：@vue/compat 长期兼容（新旧共存）执行步骤

> 本节是方案二的详细执行步骤。Phase A、B 与方案一的 Phase 1、2 完全相同（参见第 3 节），此处仅列出差异部分。
...
**目标**：切换到 @vue/compat 运行时，配置双 UI 库共存环境。新页面使用 Vue 3 + 新 UI 库，老页面保持原样不动。
...
> 本文档将随迁移推进持续更新，每个 Phase 完成后补充实际遇到的问题和解决方案。
